...
*[Proximo capitulo: 07 — Analise Estatica com CMake](07-analise-estatica.md)*
---

## 17. Deep Dive: Entendendo o ELF para Hardening

### Secoes Relevantes para Seguranca

O formato ELF (Executable and Linkable Format) contem varias secoes que sao diretamente afetadas pelo hardening. Entender essas secoes e fundamental para diagnosticar problemas e verificar protecoes.

```
# Listar todas as secoes de um binario
readelf -S /usr/bin/example

# Secoes relevantes para seguranca:
# .text       - Codigo executavel (deve ser read-only + execute)
# .data       - Dados inicializados globais (leitura-escrita)
# .bss        - Dados nao inicializados globais
# .rodata     - Constantes (read-only)
# .got        - Global Offset Table (read-only com Full RELRO)
# .got.plt    - GOT para lazy binding (read-only com Full RELRO)
# .plt        - Procedure Linkage Table
# .dynamic    - Informacoes para o linker dinamico
# .dynsym     - Tabela de simbolos dinamicos
# .dynstr     - Strings dos simbolos dinamicos
# .gnu.hash   - Hash table para busca de simbolos
# .note.GNU-stack - Indica se a stack e executavel
# .note.GNU-property - Propriedades do binario
```

### Flags ELF de Seguranca

```
# Flags de seguranca no ELF header
readelf -l /usr/bin/example | grep GNU_STACK
...
# W = Write
# E = Execute
# Sem W = NX (No-eXecute) habilitado
...
# Verificacao de RELRO
readelf -l /usr/bin/example | grep GNU_RELRO
readelf -d /usr/bin/example | grep BIND_NOW
# BIND_NOW = Full RELRO
# Sem BIND_NOW = Partial RELRO (ou sem RELRO)

# Verificacao de Stack Canary
readelf -s /usr/bin/example | grep __stack_chk
# Se presente: stack canary habilitado
# __stack_chk_guard: variavel global do canary
# __stack_chk_fail: funcao chamada em caso de smash
```

### Program Headers vs Section Headers

```
# Program headers (usados pelo loader):
readelf -l /usr/bin/example
# Mostra como o binario e carregado na memoria
# Inclui: LOAD, DYNAMIC, INTERP, NOTE, GNU_RELRO, GNU_STACK

# Section headers (usados pelo linker):
readelf -S /usr/bin/example
# Mostra todas as secoes do binario
# Inclui: .text, .data, .got, .plt, etc.

# Para hardening, program headers sao mais relevantes:
# - GNU_RELRO indica nivel de RELRO
# - GNU_STACK indica se NX esta ativo
# - LOAD indica permissoes das paginas de memoria
```

---

## 18. Deep Dive: FORTIFY_SOURCE em Detalhes

### O Que FORTIFY_SOURCE Faz

FORTIFY_SOURCE e uma extensao do GCC e Clang que substitui funcoes de string por versoes seguras que verificam o tamanho dos buffers em tempo de execucao.

```
# Niveis de FORTIFY_SOURCE:
# 1: Verificacao basica de buffers
# 2: Verificacao mais rigorosa + verificacao em funcoes de formato
# 3: Verificacao extrema (GCC 12+, Clang 16+)

# Ativacao:
# -D_FORTIFY_SOURCE=1   (basico)
# -D_FORTIFY_SOURCE=2   (recomendado)
# -D_FORTIFY_SOURCE=3   (maximo)

# Requisitos:
# - Compilacao com otimizacao (-O1 ou superior)
# - Headers do sistema devem suportar FORTIFY_SOURCE
# - Linux com glibc ou musl
```

### Funcoes Protegidas

```
# Funcoes de string protegidas:
memcpy, memmove, memccpy
strcpy, strncpy, stpcpy, stpncpy
strcat, strncat
gets (descontinuada, sempre segura)

# Funcoes de formato protegidas:
sprintf, snprintf, vsprintf, vsnprintf
fprintf, vfprintf
syslog

# Funcoes de E/S protegidas:
fgets, gets (descontinuada)
fread, fwrite

# Exemplo de verificacao em runtime:
void *dest = malloc(16);
char *src = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
memcpy(dest, src, 100);  // FORTIFY detecta: 100 > 16
// Output: *** buffer overflow detected ***: terminated
```

### Limitacoes do FORTIFY_SOURCE

```
# FORTIFY_SOURCE so funciona quando:
# 1. O tamanho do buffer e conhecido em tempo de compilacao
# 2. A funcao e chamada diretamente (nao via ponteiro)
# 3. O compilador consegue analisar o fluxo de dados

# Exemplos onde FORTIFY nao funciona:
void unsafe_copy(char *dest, char *src, size_t n) {
    // Se dest for um ponteiro generico, FORTIFY nao consegue
    // determinar o tamanho do buffer
    memcpy(dest, src, n);  // Sem verificacao
}

# FORTIFY funciona quando dest e um array:
void safe_copy() {
    char buffer[16];
    char *src = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    memcpy(buffer, src, 100);  // FORTIFY detecta
}
```

---

## 19. Deep Dive: Stack Clash Protection

### O Que e Stack Clash

Stack Clash e uma classe de vulnerabilidade onde o atacante usa alocacoes grandes na stack para "pular" por cima de guard pages e corromper memoria adjacente.

```
# Guard page e uma pagina de memoria nao mapeada entre
# a stack e outras regioes de memoria
# Se o programa tentar acessar a guard page, recebe SIGSEGV

# O ataque funciona assim:
# 1. Alocar um buffer grande na stack (ex: 1MB)
# 2. O buffer pula por cima da guard page
# 3. Escrever dados no buffer que atingem memoria adjacente
# 4. Sobrescrever dados criticos (variaveis de controle, ponteiros)

# Stack clash protection previne isso verificando:
# - Se o frame pointer esta dentro dos limites esperados
# - Se a stack nao ultrapassou guard pages
# - Se a alocacao e segura
```

### Ativando Stack Clash Protection

```
# GCC:
gcc -fstack-clash-protection -o app main.c

# Clang:
clang -fstack-clash-protection -o app main.c

# Em CMake:
add_compile_options(-fstack-clash-protection)

# Como funciona internamente:
# O compilador insere codigos que verificam o tamanho
# de cada alocacao na stack
# Se a alocacao for grande demais, o codigo aborta
```

### Protecao em Funcoes com VLAs

```
# Variaveis Length Array (VLA) sao especialmente vulneraveis
# a stack clash porque o tamanho nao e conhecido em tempo de compilacao

void vulnerable_vla(size_t n) {
    char buffer[n];  // Tamanho desconhecido
    // Stack clash pode pular guard page aqui
}

# Com stack-clash-protection:
# Compilador insere verificacao antes da alocacao
# Se n for grande demais, o programa aborta

# Alternativa segura: usar alloca ou malloc
void safer_allocation(size_t n) {
    if (n > MAX_BUFFER_SIZE) {
        return;  // Rejeitar tamanho invalido
    }
    char *buffer = alloca(n);  # Ainda usa stack, mas com verificacao
}
```

---

## 20. Deep Dive: NX/DEP e W^X

### NX (No-eXecute) Bit

O bit NX (ou XD - eXecute Disable) e uma protecao de hardware que impede a execucao de codigo em paginas de memoria marcadas como escrita.

```
# Sem NX:
# A stack e marcada como RWX (Read-Write-Execute)
# Atacante pode escrever shellcode na stack e executa-lo

# Com NX:
# A stack e marcada como RW (Read-Write)
# Shellcode na stack nao pode ser executado
# Atacante precisa usar ROP ou outros gadgets

# Verificacao:
readelf -l /usr/bin/example | grep GNU_STACK
# Flags RWE = NX desabilitado
# Flags RW  = NX habilitado
```

### W^X (Write XOR Execute)

W^X e o principio de que uma pagina de memoria pode ser escrita OU executada, mas nao ambos ao mesmo tempo.

```
# Implementacao no Linux:
# - mmap com PROT_WRITE nao pode ter PROT_EXEC
# - mprotect pode alternar entre W e X, mas nao ambos

# Exemplo de violacao W^X:
void *p = mmap(NULL, 4096, PROT_READ | PROT_WRITE, ...);
memcpy(p, shellcode, sizeof(shellcode));
mprotect(p, 4096, PROT_READ | PROT_EXEC);  # Muda para executavel
// Isso e permitido, mas W^X rigoroso impediria

# Na pratica, NX bit ja fornece W^X para paginas normais
# Stack, heap, e dados sao RW (nao X)
# Codigo e RX (nao W)
```

### Protecao Contra Return-to-libc

```
# Return-to-libc usa funcoes ja carregadas na memoria
# (como system(), execve())
# NX impede shellcode, mas nao impede return-to-libc

# Protecao contra return-to-libc:
# 1. ASLR/PIE: randomiza enderecos das bibliotecas
# 2. Full RELRO: impede sobrescrita da GOT
# 3. Stack canaries: detectam stack smash
# 4. CFI: verifica integridade do fluxo de controle

# Combinacao ideal:
# Full RELRO + ASLR/PIE + Stack Canaries + NX + CFI
```

---

## 21. Checklist de Hardening

Use este checklist para verificar se todas as protecoes estao implementadas no seu projeto:

```
=== Checklist de Hardening de Binarios ===

[ ] Stack Canaries
    [ ] -fstack-protector-strong (ou -fstack-protector-all)
    [ ] -fstack-clash-protection
    [ ] Verificar: __stack_chk_fail presente no binario

[ ] RELRO
    [ ] -Wl,-z,relro,-z,now (Full RELRO)
    [ ] Verificar: BIND_NOW no readelf -d

[ ] ASLR/PIE
    [ ] -fPIE -pie
    [ ] Verificar: Type: DYN no readelf -h

[ ] NX/DEP
    [ ] -Wl,-z,noexecstack
    [ ] Verificar: GNU_STACK sem flags de execucao

[ ] FORTIFY_SOURCE
    [ ] -D_FORTIFY_SOURCE=2
    [ ] Verificar: Compilacao com -O1 ou superior

[ ] Strip de Simbolos
    [ ] strip --strip-all para Release
    [ ] ou objcopy --only-keep-debug para debug separado
    [ ] Verificar: file binario diz "stripped"

[ ] Reproducible Builds
    [ ] SOURCE_DATE_EPOCH configurado
    [ ] -frandom-seed usado
    [ ] Variaveis de ambiente fixadas

[ ] Code Signing
    [ ] GPG ou Sigstore configurado
    [ ] Assinatura gerada apos build
    [ ] Verificacao documentada

[ ] RPATH Seguro
    [ ] $ORIGIN usado para rpath relativo
    [ ] Sem dependencia de LD_LIBRARY_PATH

[ ] Instalacao Segura
    [ ] Permissoes corretas nos arquivos
    [ ] DESTDIR usado para staging
    [ ] GNUInstallDirs para padronizacao
```

---

## 22. Erros Comuns e Como Evita-los

### Erro 1: Esquecer Full RELRO

```
# Errado:
target_link_options(myapp PRIVATE -Wl,-z,relro)
# Isso da Partial RELRO, nao Full

# Correto:
target_link_options(myapp PRIVATE -Wl,-z,relro,-z,now)
# Full RELRO com BIND_NOW
```

### Erro 2: Strip Antes de Debug

```
# Errado:
strip --strip-all app
# Apos strip, nao e possivel debugar

# Correto:
# Primeiro: compilar com -g
# Depois: separar debug info
objcopy --only-keep-debug app app.debug
strip --strip-debug app
# Agora temos app (producao) + app.debug (debug)
```

### Erro 3: RPATH Absoluto

```
# Errado:
set_target_properties(myapp PROPERTIES
    INSTALL_RPATH "/usr/local/lib"  # RPATH absoluto
)

# Correto:
set_target_properties(myapp PROPERTIES
    INSTALL_RPATH "$ORIGIN/../lib"  # RPATH relativo
)
```

### Erro 4: Nao Verificar Resultado

```
# Errado:
# Compilar com protecoes mas nao verificar se estao ativas

# Correto:
# Sempre verificar apos build:
readelf -d app | grep BIND_NOW
readelf -h app | grep Type
checksec --file=app
```

### Erro 5: Compilacao nao Reproduzivel

```
# Errado:
# Usar __DATE__ e __TIME__ no codigo
const char *build = __DATE__ " " __TIME__;

# Correto:
# Usar SOURCE_DATE_EPOCH ou timestamp do git
const char *build = SOURCE_DATE_EPOCH;
# Ou definir via variavel de ambiente
```

### Erro 6: Instalacao sem Permissoes

```
# Errado:
install(TARGETS myapp DESTINATION bin)
# Permissoes padrao podem ser inadequadas
...
## 23. Referencias Adicionais

### Livros

- **"Computer Security: Principles and Practice"** — William Stallings, Lawrie Brown. Seguranca abrangente incluindo protecao de memoria.
- **"The Art of Memory Forensics"** — Michael Hale Ligh et al. Analise de memoria e exploits.
- **"Practical Binary Analysis"** — Dennis Andriesse. Analise de binarios ELF e hardening.
- **"Linkers and Loaders"** — John Levine. Entendendo como linkers e loaders funcionam.

### Videos e Cursos

- **LiveOverflow**: Serie sobre binary exploitation e CTF
- **PwnCollege**: Cursos gratuitos de binary exploitation
- **Binary Adventure**: Analise de binarios e reverse engineering

### Comunidades

- **CTF Wiki**: https://ctf-wiki.org/ — Recursos para CTF e binary exploitation
- **Exploit Education**: https://exploit.education/ — Exercicios praticos de exploitação
- **pwn.college**: https://pwn.college/ — Cursos estruturados de seguranca
